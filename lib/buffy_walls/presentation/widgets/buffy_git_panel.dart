...
class BuffyGitPanel extends ConsumerStatefulWidget {
  const BuffyGitPanel({super.key});

  @override
  ConsumerState<BuffyGitPanel> createState() => _BuffyGitPanelState();
}

class _BuffyGitPanelState extends ConsumerState<BuffyGitPanel>
  ...
  void _refresh() {
    if (!_spinAnim.isAnimating) {
      _spinAnim.repeat();
      ref.read(buffyCmsProvider.notifier).refreshGitStatus();
  ...
  Widget build(BuildContext context) {
    final buffyState = ref.watch(buffyCmsProvider);
    final gitStatus = buffyState.gitStatus;
    final activeTarget = ref.watch(activeBuffyGitTargetProvider);
        ...
        children: [
          ...
          Expanded(
            child: AnimatedSwitcher(
              ...
              child: _buildBody(buffyState, gitStatus, colors),
            ),
          ),
        ],
      ),
  ...
  Widget _buildTargetSelector(BuffyGitRepoTarget active, AppColorScheme colors) {
    return Padding(
            ...
            child: Row(
              children: [
                _buildTargetBtn(
                  ...
                  BuffyGitRepoTarget.json, active == BuffyGitRepoTarget.json, colors,
                  isLeft: true,
                ),
                Container(width: 1, height: 20, color: colors.borderSubtle),
                _buildTargetBtn(
                  'Wallpapers', Icons.image_rounded,
                  BuffyGitRepoTarget.wallpapers, active == BuffyGitRepoTarget.wallpapers, colors,
                  isLeft: false,
                ),
              ],
            ),
  ...
  Widget _buildTargetBtn(
    ...
    return Expanded(
      child: InkWell(
        onTap: () {
          ref.read(activeBuffyGitTargetProvider.notifier).set(target);
          ref.read(buffyCmsProvider.notifier).refreshGitStatus();
        },
  ...
  Widget _buildBody(dynamic buffyState, dynamic status, AppColorScheme colors) {
    if (buffyState.isLoading && status == null) {
      return const Center(
    ...
    if (status == null) {
      return _buildEmptyState(
        ...
        'No repository configured.\nSet a local repo path in settings.',
        colors,
    ...
    return _buildStatusContent(status, colors, key: const ValueKey('content'));
  }
      ...
      children: [
        ...
        ...files.take(20).map(
              (f) => _FileItem(
                ...
                isStaged: isStaged,
                onAction: () => isStaged
                    ? ref.read(buffyCmsProvider.notifier).unstageFile(f)
                    : ref.read(buffyCmsProvider.notifier).stageFile(f),
              ),
            ),
        if (files.length > 20)
          Padding(
  ...
  Future<void> _handleCommit() async {
    ...
    final result = await ref.read(buffyCmsProvider.notifier).commit(message);
    if (mounted) {
  ...
  Future<void> _handlePush() async {
    final result = await ref.read(buffyCmsProvider.notifier).push();
    if (mounted) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(result.message)));
    }
  ...
  Future<void> _handlePull() async {
    final result = await ref.read(buffyCmsProvider.notifier).pull();
    if (mounted) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(result.message)));
    }
