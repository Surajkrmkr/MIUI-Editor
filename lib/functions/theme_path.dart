...
class CurrentTheme {
  static String? getPath(context) {
    ...
    final themeName =
        provider.paths![provider.index!].split(Platform.isWindows ? "\\" : "/").last.split('.').first;
  ...
  static Future createWallpaperDirectory({String? themePath}) async {
    await Directory(platformBasedPath("$themePath\\wallpaper\\")).create(recursive: true);
  }

  static Future createLockscreenDirectory({String? themePath}) async {
    await Directory(platformBasedPath("$themePath\\lockscreen\\advance")).create(recursive: true);
  }
}
